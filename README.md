...
Full protocol — research questions, variable tables, instrumentation list, analysis plan, and scope notes — is in [`docs/study/README.md`](docs/study/README.md). The data schema (one row per strike) is in [`docs/study/data-template.csv`](docs/study/data-template.csv); raw measurement data will live under `docs/study/build/data/` once Phase 1 collection begins.

## Skills demonstrated

See [`docs/SKILLS.md`](docs/SKILLS.md) — a human-readable agent-skill index. Three skills surface here:
...
- **`design-of-experiments-for-key-tuning`** *(stub — defined inline in `docs/study/README.md`)* — three-phase characterization framework.
- **`cantilever-flexural-frequency`** *(stub — currently inline in `docs/study/README.md`)* — `f₁ ≈ 0.162 · (h/L²) · √(E/ρ)` for tongue tuning.
...
The full move history for this layout is tracked in [`docs/reorg-manifest.json`](docs/reorg-manifest.json).
